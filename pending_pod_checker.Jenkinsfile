String mailRecipients = 'dongjin.kim@xxxx.com'
...
node {
    sshagent(['jenkins-docker']) {
        String numOfPendingContainer = sh(
                script: '''
                #!/bin/bash
                echo $(ssh xxxxx@xx.xx.xxx.xx "kubectl get pods -n jenkins | grep Pending | awk -F ' ' {'print\\$5'} | awk -v time=\\$params.THRESHOLD_TIME -F 's' {'if(\\$1>time)print\\$1'} | wc -l")
                '''
                , returnStdout: true
        ).trim()
        ...
        String report = '<h3>⚠[Kubernetes Warning] POD Scheduling Delay</h3> <p> ⌛ Number of unscheduled PODs :&nbsp;<strong>' + numOfPendingContainer + '</strong> (over than ' + params.THRESHOLD_TIME + 'sec)</p>' +
                "<br><br><a href='${env.JOB_URL}'>Go to pending POD checker</a>"
